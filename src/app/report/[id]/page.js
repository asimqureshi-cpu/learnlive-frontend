      ...
      <div style={{ display: 'flex', gap: '3px' }}>
        {[1,2,3,4,5,6].map(i => (
          <div key={i} style={{ width: '8px', height: '8px', borderRadius: '1px',
            background: i <= n ? (colors[level] || '#a89878') : '#e8e0d0' }} />
        ))}
      </div>
    ...
    <div style={{ minHeight: '100vh', background: '#faf8f3', fontFamily: "'Crimson Pro', Georgia, serif" }}>
      ...
      <header style={{ borderBottom: '1px solid #e8e0d0', background: '#fff', padding: '0 3rem',
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.75rem' }}>
          ...
          <span style={{ fontSize: '0.7rem', color: '#8b7355', fontFamily: "'DM Mono', monospace",
            letterSpacing: '0.1em', textTransform: 'uppercase' }}>Report</span>
        </div>
        {/* Export buttons */}
        <div className="no-print" style={{ display: 'flex', gap: '0.5rem' }}>
          <button
            style={btnStyle}
            onClick={() => window.open(API + '/api/reports/' + id + '/export/csv')}>
            CSV
          </button>
          <button
            style={btnStyle}
            onClick={() => window.open(API + '/api/reports/' + id + '/export/pdf')}>
            PDF
          </button>
      ...
      <main style={{ maxWidth: '860px', margin: '0 auto', padding: '3rem 2rem' }}>
        <div style={{ marginBottom: '2.5rem', paddingBottom: '2rem', borderBottom: '1px solid #e8e0d0' }}>
          ...
          <h1 style={{ fontSize: '2.4rem', fontWeight: '400', color: '#1a1208',
            letterSpacing: '-0.01em', marginBottom: '0.5rem' }}>{session.title}</h1>
        ...
        <div className="section">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
            <div>
              <p style={{ fontSize: '0.8rem', fontFamily: "'DM Mono', monospace", color: '#2d6a4f',
                letterSpacing: '0.08em', marginBottom: '0.75rem' }}>STRENGTHS</p>
            ...
            <div>
              <p style={{ fontSize: '0.8rem', fontFamily: "'DM Mono', monospace", color: '#8b3a2a',
                letterSpacing: '0.08em', marginBottom: '0.75rem' }}>GAPS</p>
          ...
          {report.group_performance?.bloom_summary && (
            <div style={{ background: '#fdfaf4', border: '1px solid #e8e0d0', borderRadius: '3px', padding: '1rem 1.25rem' }}>
              <p style={{ fontSize: '0.7rem', fontFamily: "'DM Mono', monospace", color: '#8b6914',
                letterSpacing: '0.1em', marginBottom: '0.4rem' }}>BLOOM'S TAXONOMY</p>
          ...
          {report.group_performance?.material_coverage && (
            <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #f0e8d8' }}>
              <p style={{ fontSize: '0.7rem', fontFamily: "'DM Mono', monospace", color: '#8b7355',
                letterSpacing: '0.1em', marginBottom: '0.4rem' }}>MATERIAL COVERAGE</p>
                  ...
                  <div key={name} className="score-card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start',
                      ...
                      <div>
                        ...
                        {/* FIX: was bloom_level_reached, correct field is bloom_level */}
                        {insight.bloom_level && <BloomBadge level={insight.bloom_level} />}
                      </div>
                      {scoreRow && (
                        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
                          {/* FIX: was topic_adherence_score/depth_score/material_application_score */}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                      <div style={{ background: '#f0f7f3', border: '1px solid #c8e0d4', borderRadius: '3px', padding: '0.875rem 1rem' }}>
                        <p style={{ fontSize: '0.65rem', fontFamily: "'DM Mono', monospace", color: '#2d6a4f',
                          letterSpacing: '0.1em', marginBottom: '0.35rem' }}>HIGHLIGHT</p>
                      ...
                      <div style={{ background: '#fdf3f0', border: '1px solid #e0c8c4', borderRadius: '3px', padding: '0.875rem 1rem' }}>
                        <p style={{ fontSize: '0.65rem', fontFamily: "'DM Mono', monospace", color: '#8b3a2a',
                          letterSpacing: '0.1em', marginBottom: '0.35rem' }}>GAP</p>
                    ...
                    {insight.recommendation && (
                      <div style={{ background: '#fdfaf4', border: '1px solid #e8d9b8', borderRadius: '3px', padding: '0.875rem 1rem' }}>
                        <p style={{ fontSize: '0.65rem', fontFamily: "'DM Mono', monospace", color: '#8b6914',
                          letterSpacing: '0.1em', marginBottom: '0.35rem' }}>RECOMMENDATION</p>
        ...
        {report.facilitator_notes && (
          <div className="section" style={{ background: '#1a1208', border: '1px solid #1a1208' }}>
            <p style={{ fontSize: '0.7rem', fontFamily: "'DM Mono', monospace", color: '#c9b890',
              letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>Facilitator Notes</p>
        ...
        {transcripts?.length > 0 && (
          <div className="section">
            ...
            <div style={{ maxHeight: '400px', overflowY: 'auto', paddingRight: '0.5rem' }}>
              {transcripts.map((t, i) => (
                <div key={i} style={{ display: 'flex', gap: '1rem', paddingBottom: '0.75rem',
                  marginBottom: '0.75rem', borderBottom: i < transcripts.length - 1 ? '1px solid #f0e8d8' : 'none' }}>
                  <span style={{ fontSize: '0.75rem', fontFamily: "'DM Mono', monospace", color: '#8b6914',
                    flexShrink: 0, paddingTop: '2px', minWidth: '70px' }}>{t.speaker_name}</span>
